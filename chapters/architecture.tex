...
\mario{Aqui cabe uma informação ao leitor do que compõe este capítulo. A seção tal apresenta..... etc}
...
%    \vspace*{-9pt} 
    \label{fig:cshia}
\end{figure}
Using the original work with some architectural elements modified to provide stronger security features, the first LEON3 \fpga~based implementation of \cshia~was realized. Figure \ref{fig:cshia} illustrates \mario{Comente o significado das cores, aqui ou na caption.} the first \cshia~implementation, where the full functionality originally proposed was available. All the interfaces, control, and design choices are part of this work, the security component highlighted, the FUZZY EXTRACTOR  is responsible for extracting and store the security key and is not described here. \mario{frase confusa.}  
...
\mario{Meu comentario anterior nao foi tratado. A introdução melhorou muito, mas os componentes são apresentados sem introdução, e sem uma visão top down para orientar o leitor. Posso assegurar que textos dificeis de ler e compreender irritam a banca.}
