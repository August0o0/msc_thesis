...
\mario{Aqui cabe uma informação ao leitor do que compõe este capítulo. A seção tal apresenta..... etc}\augusto{adicionado}
...
%    \vspace*{-9pt} 
    \label{fig:cshia}
\end{figure}
Using the original work with some architectural elements modified to provide stronger security features, the first \leon~\fpga~based implementation of \cshia~was realized. Figure \ref{fig:cshia} illustrates \mario{Comente o significado das cores, aqui ou na caption.}\augusto{adicionei no texto} the first \cshia~implementation, where the full functionality originally proposed was available and indicated in light blue. All the interfaces, control, and design choices are part of this work, the security component hatched in the Figure, the FUZZY EXTRACTOR  ehich is responsible for extracting and store the security key is an IP developed  in another work and wont be detailed in this dissertation. \mario{frase confusa.} \augusto{tentei deixar claro que nao é parte do trabalho}  

This basic architecture was further evaluated and expanded to be robust against replay attacks, for this,  the architecture illustrated in Figure \ref{fig:cshiaexpanded} was implemented and extra security components were added, the \ptag~cache, a timestamp generation and control and a security feature shown as MERKLE TREE control. Together these new components that are not part of this work provide a more robust solution that is evaluated in the next sessions. \mario{Agora ficou muito bom, muito claro.}
...
\mario{Meu comentario anterior nao foi tratado. A introdução melhorou muito, mas os componentes são apresentados sem introdução, e sem uma visão top down para orientar o leitor. Posso assegurar que textos dificeis de ler e compreender irritam a banca.}\augusto{coloquei introduçoes em todos os lugares faltantes para amenizar esse impacto que voce mencionou. acho que faz mais sentido uma abordagem botton up  visto que o objetivo final é falar dos modos de operação, tentei deixar isso clano na introdução do capitulo  }
